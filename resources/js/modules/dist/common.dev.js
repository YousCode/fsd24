...
Common.showFloatingEl = function (element) {
  element.removeClass('scale_down').show().addClass('scale_up'); //fadeIn(300);
  ...
  $('body').on('click', '.js-toggle-button', function () {
    ...
    if (!$thisContent.is(':visible')) {
      ...
      setTimeout(function () {
        ...
        $(_this).closest('.js-toggle-wrapper').addClass('is-active');
      }, 10);
    } else {
      Common.hideFloatingEl($('.js-toggle-content'));
      $(this).closest('.js-toggle-wrapper').removeClass('is-active');
...
Common.init = function () {// Common functions called
};

Common.test();
Common.init(); //-- Task Actions buttons

Common.actionsButtonsInteration(); //-- Toggle this Content

Common.toggleContent(); //-- Toggle active Class

Common.toggleClass(); //-- Content Position

Common.contentPosition(); //-- Popup Position

Common.popupPosition(); //-- Slide Toggle
